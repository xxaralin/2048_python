...
class Game(tk.Frame):  
    def __init__(self):
        ...
        self.main_grid=tk.Frame(self, 
            bg=c.GRID_COLOR, bd=3, width=600, height=600)
    ...
    def game_over(self):
        ...
        elif not any(0 in row for row in self.matrix) and not self.horizantal_move() and not self.vertical_move():
            game_over_frame=tk.Frame(self.main_grid,borderwidth=2)
            game_over_frame.place(relx=0.5, rely=0.5, anchor="center")
            tk.Label(game_over_frame, text="You lose!",
                bg=c.LOSER_BG,
                fg=c.GAME_OVER_FONT_COLOR,
                font=c.GAME_OVER_FONT).pack()
